...
def main():
    ...
    print("Geometrized frames")
    #frames = 101
    generate_adofai(frames)
    print("Generated adofai")
...
def geometrize(frame_begin_num, frame_end_num):
    for i in range(frame_begin_num, frame_end_num):
        os.system(f"geometrize_lib -i frames/{i}.png -o jsons/{i}_out.json -t rotated_rectangle -s " + str(DECORATION_NUM))

def generate_adofai(frame_num):
    with open('video_template.adofai', encoding='utf-8-sig') as json_file:
    ...
    for i in range(frame_num):
        with open(f'jsons/{i}_out.json') as json_file:
            ...
            for deco in data["shapes"]:
                adofai["actions"].append(deepcopy(format.set_deco_move(i * offset, deco, 0, cnt)))
                adofai["actions"].append(deepcopy(format.set_deco_attribute(i * offset, deco, 0, cnt)))
                cnt += 1
