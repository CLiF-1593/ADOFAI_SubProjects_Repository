...
Json::Value Pause(int floor, double du) {
	Json::Value bpm_change;
	bpm_change["floor"] = floor;
	bpm_change["eventType"] = "Pause";
	bpm_change["duration"] = du;
	bpm_change["countdownTicks"] = 0;
	bpm_change["angleCorrectionDir"] = -1;
	return bpm_change;
}
...
void Editor::ChangeFile() {
	...
	for (int i = this->range.begin; i <= this->range.end; i++) {
		...
		if (prev_angle != angle) {
			actions->append(Pause(i, angle / 180.0));
			prev_angle = angle;
		}
	}
}
